...
trait UserAttribute
{
    ...
    public function getPermissionsLabelAttribute()
    {
        ...
        $permissions = $permissionRepository->getEnabledPermissions($this->getDirectPermissions())->toArray();

        if (\count($permissions)) {
            return implode(', ', array_map(function ($item) {
                return __($item['package_name'].($item['package_name']?"::":"")."permissions.".$item['name']);
            }, $permissions));
        }
